...
function [x, num_iter, time, f_min] = gradDescOpt_alpha(x_init, max_iter, epsilon, a, b, c, d)
...
dist = Inf;
    ...
    while dist >= epsilon && k < max_iter % stop criteria : epsilon or iteration number
        ...
        [~,idx] = min(f0(x-g*alpha,a,b,c,d)); % choose alpha with the best descent

        x = x - alpha(idx)*g; % update rule
